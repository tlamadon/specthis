...
- Where used, a **library** entry's certificate is the natural way for
  its consumers to depend on it without the library being a step (§7.6).
...
§14, for projects with no compute manager. It walks the DAG and nothing
else.

- **Format** — `pipeline.toml`, the only pipeline format specthis both
  reads *and* has an implementation for:
...
- **Edges are derived**, DVC-style: a step follows every step whose
  `outs` its `deps` name. Optional `after = [...]` adds ordering no file
  expresses.
...
**It does not live inside the notary.** `runner.py` imports only the
standard library and the pipeline reader — never `check`, `ledger` or
`parse` — so *specthis never forks a process* remains true of the
notary while the package ships a tool that does. The boundary is
asserted in the test suite.

### 7.4 Which entries have steps
...
### 7.5 How a step's dependencies are classified
